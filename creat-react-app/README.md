# create-react-app
create-react-app 把 webpack 等設定都包好好, 你有相關的css, js, lint 甚至是 flow type check 等等

除非你用 npm run eject （會把所有設定 eject 出來, 像是 webpack 等等，讓你可以自己修改，無法塞回去喔！）否則也不能更動

其餘的用法都算簡單, 但是若是要 deploy 到 github page， router 必須要用 hashistory, 不能用browserhistory，下面有說明。

Deploy 到 github page 的script 甚至都幫你寫好了
